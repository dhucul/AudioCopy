...
void AudioCDCopier::PrintQCheckReport(const QCheckResult& result) {
	...
	if (!result.samples.empty()) {
		...
		// All verdict-driving metrics are normalised to their own peak
		// (0–100%) and overlaid on one graph.  Pioneer E22 is raw vendor
		// diagnostic data, so it is intentionally excluded from this
		// decision graph.
		std::vector<int> bucketC1 = Console::BucketData(c1Vals, GRAPH_WIDTH);
		const bool includeC2InCombined = !pioneerScan;
		std::vector<int> bucketC2 = includeC2InCombined
			? Console::BucketData(c2Vals, GRAPH_WIDTH)
			: std::vector<int>(GRAPH_WIDTH, 0);
		...
		std::cout << (pioneerScan
			? "\033[1m  Combined C1/CU Overview\033[22m\n"
			: "\033[1m  Combined C1/C2/CU Overview\033[22m\n");
		Console::SetColorRGB(Console::Theme::DimR, Console::Theme::DimG, Console::Theme::DimB);
		std::cout << (pioneerScan
			? "  Normalized verdict metrics. Pioneer E22 is not verified C2. "
		...
		for (int row = COMBINED_HEIGHT; row >= 1; row--) {
			...
			for (int col = 0; col < GRAPH_WIDTH; col++) {
				...
				else if (includeC2InCombined && normC2d >= rowFrac) {
					Console::SetColorRGB(Console::Theme::YellowR, Console::Theme::YellowG, Console::Theme::YellowB);
					std::cout << Console::Sym::Bar8;
				}
		...
		if (includeC2InCombined)
			std::cout << "C2 (peak " << result.maxC2PerSecond << ")  ";
		std::cout << "CU (peak " << result.maxCUPerSecond << ")";
		if (pioneerE22Observed)
			std::cout << "  E22 diagnostic peak " << result.maxPioneerE22PerSecond;
		std::cout << "\n";
		Console::Reset();
	}
	...
	if (result.c1Unverified) {
		...
		std::cout << "     This is extremely unlikely on any real CD.\n"
			...
			<< "     Suggestions:\n"
			<< "       - Try the C2 error scan (option 6) instead.\n"
