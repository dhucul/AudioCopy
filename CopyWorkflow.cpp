...
namespace {
// Best-effort: on Pioneer drives, enable PureRead Master + Quiet speed mode +
// Fragile/Rental CD slow-rotation mode before extracting audio. These settings
// are session-only (eepSave=false) so the drive returns to its prior state on
// next power-cycle. Silent no-op for non-Pioneer drives.
void TryApplyPioneerAudioPreset(AudioCDCopier& copier) {
    ...
    bool changed = pioneer.ApplyAudioExtractionPreset(/*persist=*/false);
    if (changed) {
        Console::Info("Pioneer drive detected — enabled PureRead Master + Quiet + Fragile CD mode for this session.\n");
    }
}
}
...
void RunWriteDiscWorkflow(AudioCDCopier& copier, const std::wstring& workDir) {
	...
	bool writeOk = copier.WriteDisc(binFile, cueFile, subFile, speed, useCal, wasBlanked);
