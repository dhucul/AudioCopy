...
int main() {
	...
	if (copier.DetectDriveCapabilities(caps)) {
		...
		if (!recommendation.empty()) {
			...
			Console::Reset();
			std::cout << "  (View full recommendations: Option 13 - Drive capabilities)\n";
		}
	}
	std::cout << "\n";
	...
	// Each iteration prints the 22-item menu (grouped by category), reads
	...
	while (true) {
		...
		PrintMenuItem(21, "Exit", true);  // Dimmed style for the exit item
		...
		case 1:
			...
			// ── 2. Write disc ───────────────────────────────────────────
			// Write disc from .bin/.cue/.sub files with rewritable detection,
			// auto-blank, and power calibration support.
		case 2:
			RunWriteDiscWorkflow(copier, dir);
			break;
		...
		case 12: {
			...
			break;
			   ...
			   // Scans disc for copy-protection mechanisms: illegal TOC,
			   // intentional errors, data tracks, subchannel manipulation, etc.
		case 13: {
			if (!hasTOC) { Console::Error("This operation requires a disc with a valid TOC.\n"); break; }
			int speed = copier.SelectScanSpeed();
			if (speed == -1) break;
			RunProtectionCheck(copier, disc, dir, speed);
			break;
		}
