...
int main() {
	...
	// Plextor SpeedRead: lift the RPC-derived read-speed cap on CD-ROM media
	// so the drive runs at its true physical maximum. No-op on non-Plextor.
	if (copier.GetDriveRef().IsPlextor()) {
		if (copier.GetDriveRef().SetSpeedRead(true)) {
			Console::Info("Plextor SpeedRead enabled.\n");
		}
	}
	...
	// ── Enter main menu ─────────────────────────────────────────────────────
	// Hand off to the 28-item interactive menu loop (MainMenu.cpp).  This
	// runs until the user selects "Exit" (option 28) or the process is
