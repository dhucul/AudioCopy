...
bool ScsiDrive::ValidateC2Accuracy(DWORD testLBA) {
	...
	constexpr DWORD CACHE_DEFEAT_DISTANCE = 5000;  // Increased from 500 for better cache clearing

	std::vector<BYTE> audio(AUDIO_SECTOR_SIZE);
	std::vector<BYTE> c2First(C2_ERROR_SIZE);
	std::vector<BYTE> c2Current(C2_ERROR_SIZE);

	// Use bit-level counting so that baselineErrors and mismatchBits share
	// the same unit, avoiding the previous byte-vs-bit threshold mismatch.
	C2ReadOptions opts;
	...
	for (int i = 0; i < NUM_READS; i++) {
		...
		// Clamp farLBA so it never underflows below 0.
		...
		if (i == 0) {
			...
			if (baselineHasC2) {
				int mismatchBits = 0;
				// Only compare the 294 actual pointer bytes
				...
				// Both baselineErrors and mismatchBits are now in bits.
				constexpr int MISMATCH_TOLERANCE = 2;
				int threshold = baselineErrors / 4 + MISMATCH_TOLERANCE;
				if (mismatchBits > threshold) {
					SetSpeed(0);
					return false;
				}
			}
			// Note: Check 1 already guarantees currentHasC2 == false here,
			// so no additional check is needed for the clean-baseline case.
		}
	}
