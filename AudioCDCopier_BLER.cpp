...
bool AudioCDCopier::RunBlerScan(const DiscInfo& disc, BlerResult& result, int scanSpeed) {
	...
	if (!errorLBAs.empty()) {
		...
		// The old proportional check (>50% of all errors) could trigger on
		// as few as 2 total errors.  Also checks inner edge, not just outer.
		...
		// three zones (inner < middle < outer) which is the hallmark of
		// spreading disc rot.  The old check skipped the middle zone entirely.
