...
int AudioCDCopier::SelectSpeed() {
	...
	if (c == 0) return -1;
	if (c == 1) { m_drive.SetSpeed(0); std::cout << "Max speed\n"; return 0; }
...
int AudioCDCopier::SelectC2Sensitivity() {
	std::cout << "\n=== C2 Detection Sensitivity ===\n";
	std::cout << "0. Back to menu\n";
	std::cout << "1. Standard (bit counting - faster)\n";
	std::cout << "2. PlexTools-style (multi-pass + cache defeat)\n";
	std::cout << "3. PlexTools-style (multi-pass, NO cache defeat - faster)\n";
	std::cout << "4. Paranoid (multi-pass with cache defeat - slowest, most accurate)\n";
	std::cout << "Choice: ";

	int c = GetMenuChoice(0, 4, 2);
	std::cin.clear(); std::cin.ignore(10000, '\n');
	if (c == 0) return -1;

	const char* modes[] = { "", "Standard mode", "PlexTools-style (multi-pass + cache defeat)",
		"PlexTools-style (multi-pass, no cache defeat)", "Paranoid (multi-pass)" };
	std::cout << modes[c] << " selected\n";
	return c;
}
...
int AudioCDCopier::SelectScanSpeed() {
	...
	std::cout << "0. Back to menu\n";
	std::cout << "1. 4x (recommended for damaged discs)\n";
	std::cout << "2. 8x (balanced)\n";
	std::cout << "3. 16x (faster, less accurate)\n";
	std::cout << "4. Max speed\n";
	std::cout << "Choice: ";
	int c = GetMenuChoice(0, 4, 2);
	std::cin.clear(); std::cin.ignore(10000, '\n');
	switch (c) {
	case 0: return -1;
	case 1: std::cout << "Using 4x speed\n"; return 4;
	case 2: std::cout << "Using 8x speed\n"; return 8;
	case 3: std::cout << "Using 16x speed\n"; return 16;
	case 4: std::cout << "Using max speed\n"; return 0;
	default: return 8;
	}
}
...
int AudioCDCopier::SelectSecureRipMode() {
	...
	std::cout << "0. Back to menu\n";
	std::cout << "1. BURST (fastest - no verification, max speed)\n";
	std::cout << "2. Standard (single pass with retry on error)\n";
	std::cout << "3. Secure Fast (2 passes, re-read on mismatch)\n";
	std::cout << "4. Secure Standard (3 passes, require 2 matches) [RECOMMENDED]\n";
	std::cout << "5. Secure Paranoid (up to 8 passes, require 3 matches)\n";
	std::cout << "Choice: ";

	int c = GetMenuChoice(0, 5, 4);
	std::cin.clear(); std::cin.ignore(10000, '\n');

	switch (c) {
	case 0: return -1;  // ✓ Change from INT_MIN to -1
	case 1: std::cout << "BURST MODE - Maximum speed, no verification\n"; return -2;  // ✓ Change to -2
	case 2: std::cout << "Standard mode - Single pass\n"; return 0;
	case 3: std::cout << "Secure FAST mode\n"; return 1;
	case 4: std::cout << "Secure STANDARD mode\n"; return 2;
	case 5: std::cout << "Secure PARANOID mode\n"; return 3;
	default: return 2;
	}
}
...
int AudioCDCopier::SelectSession(int sessionCount) {
	...
	std::cout << "0. All sessions\n";
	for (int i = 1; i <= sessionCount; i++) {
		std::cout << i << ". Session " << i << "\n";
	}
	std::cout << "Choice: ";

	int c = GetMenuChoice(-1, sessionCount, 0);  // ✓ Changed minChoice from 0 to -1
	std::cin.clear(); std::cin.ignore(10000, '\n');
	if (c == -1) return -1;  // ✓ Change from -2 to -1 for consistency
	return c;
}
