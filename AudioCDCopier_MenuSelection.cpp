...
int AudioCDCopier::SelectHideCDRMedia() {
...
int AudioCDCopier::SelectSilentMode() {
...
int AudioCDCopier::SelectPlextorWriteOptions(bool& outTestWrite,
	...
	std::cout << "Choice: ";
	int v = GetMenuChoice(0, 2, 2);
	std::cin.clear(); std::cin.ignore(10000, '\n');
	if (v == 0) return -1;

	if (v == 1) {
		outVariRecEnable = true;
		std::cout << "Power offset (-4 to +4, 0 = no change): ";
		std::string line;
		std::getline(std::cin, line);
		try {
			int n = std::stoi(line);
			if (n < -4 || n > 4) {
				Console::Warning("Out of range — using 0.\n");
				n = 0;
			}
			outVariRecOffset = n;
		}
