...
bool RunTrackRipWorkflow(AudioCDCopier& copier, DiscInfo& disc, const std::wstring& /*workDir*/) {
	...
	if (!isBurst) {
		...
		if (copier.DetectDriveCapabilities(caps)) {
			...
			// Pioneer-only: enable PureRead Master + Quiet + Fragile CD mode for this session.
			{
		...
		// Pioneer-only: enable PureRead Master + Quiet + Fragile CD mode for this session.
		// Matches the Copy workflow (option 1), which detects capabilities and applies the
		// preset for both burst and safe rips.
		std::cout << "\nDetecting drive capabilities..." << std::flush;
		DriveCapabilities caps;
		if (copier.DetectDriveCapabilities(caps)) {
			std::cout << " done.\n";
